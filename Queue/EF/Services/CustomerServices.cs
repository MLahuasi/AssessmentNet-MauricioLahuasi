    ...
    public class CustomerServices
    {
        ...
        public async Task<IEnumerable<Customer>> GetCustomers()
        {
            return await _context.Customers
                .Where( c => c.Status == Transacction.Command.Action.NoAttend)
                .ToListAsync();
        }
        ...
        public async Task<List<QueueInfo>?>  GetMinQueueWithCustomer ()
        {
            ...
            .Select(cq => new QueueInfo
            {
                QueueId = cq.Id,
                QueueName = cq.Name,
                CustomerCount = cq.Customers.Count(c => c.Status == Transacction.Command.Action.NoAttend),
                TotalQueueDuration = cq.Customers.Where(c => c.Status == Transacction.Command.Action.NoAttend).Sum(c => c.Queue.Duration) 
            })
            .ToListAsync();
        ...
        {
            var result = await _context.CustomersQueue
            .Where(cq => cq.Customers.Any(c => c.Status == Transacction.Command.Action.NoAttend))
            .Select(cq => new
            {                
                Customers = cq.Customers.Where(c => c.Status == Transacction.Command.Action.NoAttend)
                    .Select(c => new
                    {
                        QueueId = cq.Id,
                        QueueName = cq.Name,
        ...
        public async Task<Customer?> Add(string ci, string name)
        {
            ...
            customer.Status = Transacction.Command.Action.NoAttend;
        ...
        public async Task<bool> Delete(string ci, string name)
        {
            ...
            currentCustomer.Status = Transacction.Command.Action.Attend;
